 ...
 * re-subscription upon any error. The re-subscription policy of this implementation is an attempt to
 * optimize and resume streaming from the last processed entry. If and only if we can't resume from the last
 * processed entry, we subscribe from the latest position in the log, which can incur in data loss, as the latest
 * position in the log might have progressed beyond several updates from the last processed entry.
 *
 * Please note if data loss is a concern DO NOT USE this listener implementation and refer to StreamListenerResumeOrFullSync
 *
...
public abstract class StreamListenerResumeOrDefault extends StreamListenerResumePolicy {
    ...
    public StreamListenerResumeOrDefault(CorfuStore store, String namespace, String streamTag, List<String> tablesOfInterest) {

...
public class StreamingPatternsIT extends AbstractIT {
    ...
    private Table<SampleSchema.Uuid, SampleTableAMsg, SampleSchema.Uuid> tableDefault;
    ...
    private class StreamListenerResumeOrDefaultImpl extends StreamListenerResumeOrDefault {
        ...
        public StreamListenerResumeOrDefaultImpl(CorfuStore store, String namespace, String streamTag, int updatesToError,
                                                 CountDownLatch errorNotifierLatch) {
    ...
    private class StreamListenerResumeOrFullSyncImpl extends StreamListenerResumeOrFullSync {
        ...
        public StreamListenerResumeOrFullSyncImpl(CorfuStore store, String namespace, String streamTag,
                                                  int updatesToError, CountDownLatch errorNotifierLatch, boolean failOnFullSync) {
            super(store, namespace, streamTag, null);
            this.updatesToError = updatesToError;
            this.errorNotifierLatch = errorNotifierLatch;
        ...
        public void onNext(CorfuStreamEntries results) {
            ...
            results.getEntries().values().forEach(entryList -> entryList.forEach(entry -> updates.add(entry)));
        }
        ...
        protected Timestamp performFullSync() {
            ...
            try (TxnContext txn = store.txn(namespace)) {
                ...
                table.entryStream().forEach(entry -> fullSyncBuffer.add(entry));
                return txn.commit();
            }
        }
    }
    ...
    public void testResumePortionOfResumeOrDefaultPolicy() throws Exception {
        ...
        StreamListenerResumeOrDefaultImpl defaultStreamListener = new StreamListenerResumeOrDefaultImpl(store, namespace,
                defaultTag, processedBeforeError, errorLatch);
        ...
        // Generate new updates (while still not re-subscribed after onError) - to confirm it resumes from the last point
        // Generate some random (interleaved entries to another table) - to confirm it is not polluting the listeners.
    ...
    public void testDefaultPortionOfResumeOrDefaultPolicy() throws Exception {
        ...
        CountDownLatch errorLatch = new CountDownLatch(2);
        StreamListenerResumeOrDefaultImpl defaultStreamListener = new StreamListenerResumeOrDefaultImpl(store, namespace,
                defaultTag, numUpdates - deltaToError, errorLatch);
        ...
        Token trimPoint = new Token
                (lastProcessedTs.getEpoch(), lastProcessedTs.getSequence() + deltaToError + (numUpdates/2));
        ...
        // Confirm no further updates have been processed, as the default policy should subscribe from the end of the log
        assertThat(defaultStreamListener.getUpdates()).hasSize(numUpdates - deltaToError);
    ...
    public void testResumePortionOfResumeOrFullSyncPolicy() throws Exception {
        ...
        // Create default stream listener
        CountDownLatch errorLatch = new CountDownLatch(2);
        StreamListenerResumeOrFullSyncImpl defaultStreamListener = new StreamListenerResumeOrFullSyncImpl(store, namespace, defaultTag, processedBeforeError, errorLatch);
    ...
    public void testFullSyncPortionOfResumeOrFullSyncPolicy() throws Exception {
        ...
        // Create default stream listener
        CountDownLatch errorLatch = new CountDownLatch(2);
        StreamListenerResumeOrFullSyncImpl defaultStreamListener = new StreamListenerResumeOrFullSyncImpl(store, namespace,
                defaultTag, numUpdates - deltaToError, errorLatch);
        ...
        // Confirm no updates have been processed (as it was reset by full sync and all updates are part of the checkpoint)
    ...
    public void testFullSyncFailureOfResumeOrFullSyncPolicy() throws Exception {
        ...
        CountDownLatch errorLatch = new CountDownLatch(2);
        StreamListenerResumeOrFullSyncImpl defaultStreamListener = new StreamListenerResumeOrFullSyncImpl(store, namespace,
                defaultTag, numUpdates - deltaToError, errorLatch, true);
